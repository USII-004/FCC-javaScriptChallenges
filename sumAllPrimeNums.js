// A prime number is a whole number greater than 1 with exactly two divisors: 1 and itself. 
// For example, 2 is a prime number because it is only divisible by 1 and 2. In contrast, 4 is 
// not prime since it is divisible by 1, 2 and 4.

// Rewrite sumPrimes so it returns the sum of all prime numbers that are less than or equal to 
// num.



function sumPrimes(num) {
  let  primeNums = [];

  // Loop through numbers from 2 to num
  for (let i = 2; i <= num; i++) {
    let isPrime = true;

    // Check if num is divisible by any number from 2 to the square root of num
    for (let j = 2; j <= Math.sqrt(i); j++) {
      if (i % j === 0) {
        isPrime = false;
        break;
      }
    }

    // If num is not divisible by any number, it is a prime number
    if (isPrime) {
      primeNums.push(i);
    }

  }

  // Sum up the prime numbers using reduce
  return primeNums.reduce((a, b) => a +b, 0);
}

sumPrimes(10);